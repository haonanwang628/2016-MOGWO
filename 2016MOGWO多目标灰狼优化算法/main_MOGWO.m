...
GreyWolves_num=100;    % Population Size (Number of Sub-Problems)
Archive_size=100;   % Repository Size
...
for ff=[7];
   ...
   for testi=1:Num_Test   %%%% 控制每次实验测试次数
       ...
       for kk=1:30 %%%% 控制实验次数的循环
           ...
           [ ub,lb,nVar ] = generate_boundary1( problem_name,m );%Upper and Lower Bound of Decision Variables  %%%生成决策空间中变量上界、下界和维度
...
%     GreyWolves(i).Cost=fobj(GreyWolves(i).Position')';
    GreyWolves(i).Cost=test_function(GreyWolves(i).Position,nVar,m,problem_name);
